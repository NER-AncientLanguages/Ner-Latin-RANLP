...
# def aggregate_ents(orig_tokens, wp_tokens, labels):
#     try:
#         assert len(wp_tokens) == len(labels)
#     except AssertionError:
#         print('lenght tokens labels are not equal')
#         print(wp_tokens)
...
# test =  [['Gallia', 'est', 'omnis', 'divisa', 'in', 'partes', 'tres', ',', 'quarum', 'unam', 'incolunt', 'Belgae'], ['Quousque', 'tandem', 'abutere', ',', 'Catilina', 'patientia', 'nostra', '?']]
...
def find_accepted_ids(sent):
    ...
    it = iter(sent)
    for i in it:
        if isinstance(i['id'], int) is False:
            accepted_ids.append(i['id'])
            lst = list(i['id'])
            del lst[1::2] #id for the non split tokens 7-8, stored in tuple (7, -, 8)
            #casting to list and deleting '-' in case there are more than two split words (10-11-12)
...
for file in conllu_files:
    ...
    for sent in sents:
        ...
        #note: ConLLu library uses OrderedDicts
        try:
